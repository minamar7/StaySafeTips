window.QuizEngine = {
  ...
  async start(lang = "el", badgeId = "quiz") {
    ...
    try {
      ...
      // FIX 1: Καθαρισμός παλιών UI στοιχείων πριν ξεκινήσει νέο quiz
      this._cleanupQuizUI();
      ...
      // FIX 1: Αν αποτύχει το fetch, δείξε χρήσιμο μήνυμα αντί να κολλήσει
      const qBox = document.getElementById("quiz-question");
      if (qBox) qBox.innerHTML = `<p style="color:var(--danger);text-align:center;font-weight:bold;">⚠️ ${lang === 'el' ? 'Σφάλμα φόρτωσης. Δοκίμασε ξανά.' : 'Load error. Please retry.'}</p>`;
    }
  },

  /* ─── FIX 1: Καθαρισμός UI στοιχείων που μένουν από προηγούμενο quiz ─── */
  _cleanupQuizUI() {
    ["quiz-timer", "quiz-streak-badge", "quiz-progress-fill"].forEach(id => {
  ...
  render() {
    ...
    // FIX 1: Guard — αν δεν υπάρχουν τα βασικά elements, σταμάτα εδώ
    if (!qBox || !oBox) {
      console.error("Quiz: missing DOM elements (quiz-question or quiz-options)");
      return;
    }
    ...
    qBox.innerHTML =
      `<p class="q-text" style="font-size:1.1rem;font-weight:bold;text-align:center;color:#ffffff;">${qData.q}</p>`;
  ...
  _updateTimerUI(remaining, total) {
    // FIX 1: Ψάχνουμε πρώτα το υπάρχον timer element
    let el = document.getElementById("quiz-timer");
    if (!el) {
      const qBox = document.getElementById("quiz-question");
      if (!qBox || !qBox.parentNode) return; // Guard: αν δεν υπάρχει parent, μην κρασάρει
  ...
  _renderStreakBadge() {
    ...
    if (!el) {
      ...
      const pill = document.getElementById("quiz-pill");
      if (pill && pill.parentNode) pill.parentNode.insertBefore(el, pill.nextSibling);
    }
    if (this.streak >= 3) {
  ...
  /* ─── SHOW RESULT ─── */
  // FIX 2: Το interstitial δείχνει ΠΡΩΤΑ το ad, μετά το result UI
  showResult() {
    this._stopTimer();
    ...
    // FIX 2: Αν είναι free user, δείξε interstitial ΠΡΩΤΑ
    // Χρησιμοποιούμε callback pattern για να περιμένουμε
    if (!premium) {
      this._showInterstitialThenResult();
    } else {
      this._renderResult();
    }
  },

  /* ─── FIX 2: Interstitial με callback — δείχνει result μόνο αφού κλείσει το ad ─── */
  _showInterstitialThenResult() {
    // Το AndroidAdmob πρέπει να καλεί window.QuizEngine._onAdClosed() όταν κλείσει το ad
    // Αν δεν κλείσει μέσα σε 8 δευτερόλεπτα, προχωράμε αυτόματα (safety net)
    const fallbackTimer = setTimeout(() => {
      console.log("Ad timeout — showing result anyway");
      this._renderResult();
    }, 8000);

    // Αποθηκεύουμε το timer για να το ακυρώσουμε αν έρθει το callback
    this._adFallbackTimer = fallbackTimer;

    try {
      if (window.AndroidAdmob && typeof AndroidAdmob.showInterstitial === "function") {
        AndroidAdmob.showInterstitial(); // Πρέπει να καλεί QuizEngine._onAdClosed() μετά
      } else {
        // Δεν υπάρχει ad (dev mode ή browser) — προχώρα αμέσως
  ...
  /* ─── FIX 2: Αυτή η συνάρτηση καλείται από το Android WebView μετά το κλείσιμο του ad ─── */
  _onAdClosed() {
    if (this._adFallbackTimer) {
      clearTimeout(this._adFallbackTimer);
      this._adFallbackTimer = null;
    }
    this._renderResult();
  },

  /* ─── Render του Result UI (χωριστά από το ad logic) ─── */
  _renderResult() {
    const res       = document.getElementById("quiz-result");
  ...
  /* ─── FIX 3: Reward video — XP δίνεται ΜΟΝΟ αν το ad ολοκληρωθεί ─── */
  _pendingRewardXP: 0,

  _requestRewardVideo(xpAmount) {
    this._pendingRewardXP = xpAmount;

    try {
      if (window.AndroidAdmob && typeof AndroidAdmob.showRewardedVideo === "function") {
        // FIX 3: Το Android πρέπει να καλεί QuizEngine._onRewardEarned() μόνο αν ο χρήστης είδε το video
        AndroidAdmob.showRewardedVideo();
      } else {
        // Dev mode / browser — δίνουμε XP αμέσως
        this._onRewardEarned();
      }
    } catch (err) {
      console.log("RewardedVideo error:", err);
      // Safety: δίνουμε XP αν κρασάρει το SDK
      this._onRewardEarned();
    }
  },

  /* ─── FIX 3: Καλείται από Android WebView ΜΟΝΟ αν ο χρήστης είδε το video ─── */
  _onRewardEarned() {
    const xp = this._pendingRewardXP || 50;
    this._pendingRewardXP = 0;
    this.updateXP(xp);
    if (window.showScreen) window.showScreen('home');
    else window.location.reload();
  },

  /* ─── Παλιό playRewardVideo — διατηρείται για backward compatibility ─── */
  playRewardVideo(xpAmount) {
    this._requestRewardVideo(xpAmount);
  },
