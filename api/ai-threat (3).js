...
export default async function handler(req, res) {
    ...
    if (!process.env.Travel_Ai_Threat) {
...
Use short bullet points with relevant emojis. Be factual, helpful, and not alarmist. If no destination is mentioned, ask which country or city they want to know about. Keep responses under 300 words.`;
    ...
    try {
        ...
        // gemini-2.0-flash-001 — stable versioned model name
        const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-001:generateContent?key=${process.env.Travel_Ai_Threat}`;
        ...
        // If Gemini returns an HTTP error, capture it
        if (!response.ok) {
